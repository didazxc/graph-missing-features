from amgraph.data import load_data
import torch
...
def num_components(node_x, edges):
    ...
    num_components = sp.csgraph.connected_components(adj, connection="weak", return_labels=False)
    return num_components

def homophily(node_x, edges):
    mean = node_x.mean(dim=0)
    std = node_x.std(dim=0)
    x = node_x  # (node_x - mean)/std
    x[torch.isinf(x)]=0
    x[torch.isnan(x)]=0
...
def main():
    ...
    for data_name in data_names:
        ...
        print(f'{data_name:10s} {homophily(node_x, edges):{len("homophily")}.4f} {num_components(node_x, edges):{len("num_components")}d} {assortativity(edges):{len("assortativity")}.4f}', )
        if need_draw_spring: draw(data_name, x=node_x, edge_index=edges)
        if need_draw_spectral: draw_spectral(data_name, x=node_x, edge_index=edges)
